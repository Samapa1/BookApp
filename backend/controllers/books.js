...
router.delete("/:id", tokenExtractor, async (req, res) => {
  if (req.user.admin !== true) {
    return res
      .status(403)
      .json({ error: "Only admins are allowed to delete books." });
  }
  ...
  await sequelize.transaction(async (t) => {
    ...
    if (book) {
      if (book.loans)
        await Promise.all(
          book.loans.map((loan) => loan.destroy({ transaction: t })),
        );

      if (book.reservations)
        await Promise.all(
          book.reservations.map((reservation) =>
            reservation.destroy({ transaction: t }),
          ),
        );

      if (book.ratings)
        await Promise.all(
          book.ratings.map((rating) => rating.destroy({ transaction: t })),
        );
